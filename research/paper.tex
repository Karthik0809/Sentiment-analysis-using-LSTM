...
\begin{itemize}
  \item \textbf{LSTM-Baseline}: 2-layer unidirectional LSTM, last hidden
        state, $h = 256$, $d_e = 128$.
  \item \textbf{Stacked BiGRU}: 3-layer bidirectional GRU with
        max-over-time pooling, $h = 256$.
  \item \textbf{BiLSTM-SA (proposed)}: as described in
        Section~\ref{sec:method}.
\end{itemize}

We fine-tune DistilBERT (\texttt{distilbert-base-uncased}) as the
primary deployed model, and evaluate BiLSTM-SA as a lightweight
comparison.
