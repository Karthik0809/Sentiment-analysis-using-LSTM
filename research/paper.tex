...
normalization that achieves a test accuracy of \textbf{90.3\%} and a
macro-averaged F1-score of \textbf{0.891}, outperforming a standard
LSTM baseline (87.4\%) and a Stacked Bidirectional GRU (89.1\%).
Ablation studies confirm the individual contributions of
bidirectionality (\texttt{+1.8\%} accuracy), attention (\texttt{+1.2\%}
accuracy), and layer normalization (\texttt{+0.6\%} accuracy). We
further compare our approach against a fine-tuned DistilBERT model and
demonstrate that the proposed architecture achieves competitive
performance at a fraction of the inference cost. The trained model is
...
We additionally fine-tune DistilBERT (\texttt{distilbert-base-uncased})
...
\midrule
LSTM-Baseline   & 0.874 & 0.861 & 0.942 & 6.7 M \\
Stacked BiGRU   & 0.891 & 0.878 & 0.958 & 9.1 M \\
BiLSTM-SA (ours) & \textbf{0.903} & \textbf{0.891} & \textbf{0.968} & 12.4 M \\
\midrule
DistilBERT\textsuperscript{†} & 0.918 & 0.906 & 0.981 & 66 M \\
\bottomrule
\multicolumn{5}{l}{\textsuperscript{†} Fine-tuned; shown for reference only.}
\end{tabular}
\end{table}

Table~\ref{tab:results} shows that BiLSTM-SA outperforms both recurrent
baselines on all metrics while remaining $\sim$5$\times$ smaller than
DistilBERT. The gap in accuracy (1.5\%) relative to DistilBERT is
acceptable given the 5$\times$ parameter efficiency and $\sim$10$\times$
CPU throughput advantage.

\subsection{Ablation Study}

\begin{table}[h]
\caption{Ablation Study (Test Accuracy)}
\label{tab:ablation}
\centering
\begin{tabular}{lc}
\toprule
...
\midrule
BiLSTM-SA (full model) & 0.903 \\
\ -- Layer Normalization & 0.897 \\
\ -- Self-Attention (last state) & 0.885 \\
\ -- Bidirectional (unidirectional) & 0.867 \\
\ -- Attention \& Bidirectional (LSTM baseline) & 0.874 \\
\bottomrule
\end{tabular}
\end{table}

As shown in Table~\ref{tab:ablation}, each component contributes
positively to the final performance. Removing bidirectionality causes
the largest single drop ($-3.6\%$), confirming that forward-backward
context is critical for short headlines. Attention contributes
$+1.8\%$ over the unidirectional last-state baseline, and layer
normalization provides a further $+0.6\%$.
