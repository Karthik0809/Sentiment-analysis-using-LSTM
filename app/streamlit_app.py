...
def predict(text: str, model, preprocessor, device):
    seq = preprocessor.encode(text)
    tensor = torch.tensor([seq], dtype=torch.long).to(device)
    with torch.no_grad():
        logits, attn_weights = model(tensor)
        probs = torch.softmax(logits, dim=1).squeeze().cpu().numpy()
    pred = int(probs.argmax())
    tokens = preprocessor.preprocess(text)[: preprocessor.max_len]
    attn = (
...
st.caption(
    "Powered by a Bidirectional LSTM with Self-Attention trained on "
...
try:
    ...
    st.warning(
        ...
        "Run `python scripts/train.py` to train and save a checkpoint."
    )
        ...
        if headline.strip():
            with st.spinner("Analysing…"):
                pred_idx, probs, tokens, attn = predict(
                    headline, model, preprocessor, device
                )
...
with tab_batch:
    ...
    if st.button("Analyse All", disabled=not model_available) and batch_input.strip():
        ...
        for i, line in enumerate(lines):
            idx, probs, _, _ = predict(line, model, preprocessor, device)
            results.append({
                "Headline":   line,
            ...
            if not headlines:
                ...
                with st.spinner(f"Analysing {len(headlines)} headlines…"):
                    headlines = analyze_headlines(
                        headlines, model, preprocessor, device
                    )
...
# ──────────────────────────────────────────────────────────────────────
with tab_about:
    st.subheader("BiLSTM + Self-Attention Architecture")
    st.markdown("""
    ```
